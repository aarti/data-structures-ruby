...
class MultiSetTest < MiniTest::Unit::TestCase
  ...
  def test_empty
    assert_equal MultiSet.new, MultiSet.new([1,2]).empty!
    assert_equal MultiSet.new, MultiSet.new.empty!
  end

  def test_multiplicity
    assert_equal 2,MultiSet.new([1, 1, 2]).multiplicity(1)
    assert_equal nil,MultiSet.new([1, 1, 2]).multiplicity(4)
    assert_equal nil,MultiSet.new([1, 1, 2]).multiplicity(1.0)
  ...
  def test_union
    ...
    assert_equal MultiSet.new([3,1]),
      MultiSet.new([1,3]) | (MultiSet.new)
    assert_equal MultiSet.new([2]),
      MultiSet.new([2]) | (MultiSet.new)
    assert_equal MultiSet.new([]),
      MultiSet.new | (MultiSet.new)
  end
